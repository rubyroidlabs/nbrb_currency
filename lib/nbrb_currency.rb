...
class NbrbCurrency < Money::Bank::VariableExchange
  NBRB_RATES_URL = 'http://nbrb.by/Services/XmlExRates.aspx'.freeze
  CURRENCIES = %w[AUD BGN UAH DKK USD EUR PLN JPY IRR ISK CAD CNY
                  KWD MDL NOK RUB SGD KGS KZT TRY GBP CZK SEK CHF].freeze
  ...
  def exchange_with(from, to_currency)
    ...
    subunits_ratio = Money::Currency.wrap(to_currency).subunit_to_unit.to_f / from.currency.subunit_to_unit.to_f
    Money.new((subunits_ratio * from.cents * rate).round, to_currency)
  end

  protected

  def exchange_rates(cache = nil)
    rates_source = cache || NBRB_RATES_URL
